...
class SankeyDiagramTool {
    ...
    initializeEventListeners() {
        ...
        document.getElementById('text-data').addEventListener('input', () => {
            ...
            this.saveDataToCookie(); // データをクッキーに保存
        });
    ...
    saveDataToCookie() {
        ...
        const unit = document.getElementById('unit-field').value;

        this.setCookie('sankey_text_data', textData, 365); // 1年間保存
        this.setCookie('sankey_unit', unit, 365);

        console.log('データをクッキーに保存しました');
    }

    // 保存されたデータを読み込み
    loadSavedData() {
        const savedTextData = this.getCookie('sankey_text_data');
        const savedUnit = this.getCookie('sankey_unit');

        if (savedTextData && savedTextData.trim()) {
            // 保存されたデータがある場合はそれを使用
            document.getElementById('text-data').value = savedTextData;
            this.unit = savedUnit || '円';
            document.getElementById('unit-field').value = this.unit;

            this.parseTextData();
            console.log('保存されたデータを復元しました');
        } else {
            // 保存されたデータがない場合はサンプルデータを使用
            this.loadSampleData();
        }
